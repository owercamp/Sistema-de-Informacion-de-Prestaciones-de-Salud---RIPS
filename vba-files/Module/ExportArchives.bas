...
Sub importInfo()
    ...
    If Dir(route & "\" & UCase(monthNow(montNumber)), vbDirectory) = Empty Then
        MkDir (route & "\" & UCase(monthNow(montNumber)))
        MkDir (route & "\" & UCase(monthNow(montNumber)) & "\IMEDICAL")
        MkDir (route & "\01" & UCase(monthNow(montNumber)))
        ActiveWorkbook.SaveCopyAs Filename:=route & "\" & UCase(monthNow(montNumber)) & "\" & "Reporte_" & year(Date) & "_" & UCase(monthNow(montNumber)) & "_1.0.xlsb"
    End If
